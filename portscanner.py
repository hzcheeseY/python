...
queue = Queue.Queue()  #多线程模块定义 存储任务队列
mutex = threading.Lock() #多线程环境控制共享资源
TIMEOUT = 10
I = 0 #任务序号
USER_DIC = {
    ...
    "mysql":['root'],
    "mssql":['sa'],
    "telnet":['administrator','admin','root','cisco','huawei','zte'],
    "postgresql":['postgres','admin']
} #存储常见用户名
PASSWORD_DIC = ['123456','admin','root','password','111111','888888','123123','123','1','{user}','{user}{user}','{user}1','{user}123','{user}2016','{user}2015','{user}!','','P@ssw0rd!!','qwa123','12345678','test','123qwe!@#','123456789','123321','1314520','666666','woaini','fuckyou','000000','1234567890','8888888','qwerty','1qaz2wsx','abc123','abc123456','1q2w3e4r','123qwe','159357','p@ssw0rd','p@55w0rd','password!','p@ssw0rd!','password1','r00t','tomcat','apache','system','huawei','zte']
#存储常见密码
REGEX = [['ftp', '21', '^220.*?ftp|^220-|^220 Service|^220 FileZilla'],  ['telnet', '23', '^\\xff[\\xfa-\\xfe]|^\\x54\\x65\\x6c|Telnet'],['mssql', '1433', ''], ['mysql', '3306', '^.\\0\\0\\0.*?mysql|^.\\0\\0\\0\\n|.*?MariaDB server'], ['postgresql', '5432', '']]
#存储服务协议匹配规则，用于判断端口开放的服务类型
class Crack():
    ...
    def run(self):
        ...
        for user in user_list:
            for pass_ in PASSWORD_DIC:
                ...
                #遍历用户名和密码列表中的每个组合，调用相应的服务攻击方法，传递用户名和密码参数
    def ftp(self,user,pass_):
        try:
            ...
            pass
        #尝试使用给定的用户名和密码连接到FTP服务，返回一个字符串，指示身份验证是否成功
    def mysql(self,user,pass_):
        ...
        if result == "\x07\x00\x00\x02\x00\x00\x00\x02\x00\x00\x00":
            return "username:%s,password:%s" % (user,pass_)
    #示例方法 发送一个包含基于用户名和密码生成的身份验证数据的数据包到MySQL服务，等待服务器的响应
    def postgresql(self,user,pass_):
        try:
            sock=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
            sock.connect((self.ip,int(self.port)))
            #创建一个套接字对象并连接到目标主机和端口
    ...
    def mssql(self,user,pass_):
        try:
            ...
            #使用TDS协议生成一个16进制字符串data，其中包括连接到MS-SQL服务所需的各种参数，例如IP地址，用户名和密码
            ...
            #将IP地址和端口号的占位符替换成真正的值，将用户名和密码的占位符替换成它们的十六进制
            ...
            return 3
        #如果响应包含单词master，则该方法返回一个成功消息，其中包含用户名和密码
    def telnet(self,user,pass_):
        try:
            ...
            return 3
        #创建Telnet对象并连接到指定的IP地址和端口来连接到Telnet服务器
    ...
    #使用上面代码提取到的用户名，密码，随机字符串和名称生成用于认证的数据，返回二进制结果
...
class SendPingThr(threading.Thread):
    ...
    #向每个ip地址发送ping包，等待timeout秒后结束
class Nscan:    
    def __init__(self, timeout=3):
        self.timeout = timeout
        self.__data = struct.pack('d', time.time()) #获取当前时间
        self.__id = os.getpid() #获取进程id
    ...
    def run(self):
        while True:
            ...
            try:
                ...
                continue
#定义一个线程类，用于多线程处理任务。将任务分为端口扫描和密码破解两种，分别使用不同函数处理
def scan_port(host,port):
    try:
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((str(host),int(port)))
        log('portscan',host,port) #连接成功，记录日志
    except Exception,e:
        return False
    try:
        data = sock.recv(512)
        if len(data) > 2:
            ...
            if len(data) > 2:
                ...
                return 'NULL'
            #发送一个空行，若返回data长度大于2，则端口开放并记录信息
    except Exception,e:
        sock.close()
        return 'NULL'
...
if __name__=="__main__":
    msg = '''
Usage: python portscanner.py -h 127.0.0.1 [-p 21,80,3306] [-m 50] [-t 10] [-d pass.txt] [-n]
    '''
    if len(sys.argv) < 2:
        print msg
    try:
        ...
        ip = ''
        port = '21,23,80,135,443,445,1433,3306,5432,6379,8080'
        m_count =50
        ping = True
